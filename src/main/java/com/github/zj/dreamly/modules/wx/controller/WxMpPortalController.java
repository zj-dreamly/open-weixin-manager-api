...
public class WxMpPortalController {
    ...
    public String authGet(@PathVariable String appid,
        ...
        logger.info("\n[MP]接收到来自微信服务器的认证消息：[{}, {}, {}, {}]", signature,
            timestamp, nonce, echostr);
        if (StringUtils.isAnyBlank(signature, timestamp, nonce, echostr)) {
            throw new IllegalArgumentException("[MP]请求参数非法，请核实!");
        }
    ...
    public String post(@PathVariable String appid,
		...
		logger.debug("\n[MP]接收微信消息：[openid=[{}], [signature=[{}], encType=[{}], msgSignature=[{}],"
						+ " timestamp=[{}], nonce=[{}], requestBody=[\n{}\n] ",
				openid, signature, encType, msgSignature, timestamp, nonce, requestBody);
        this.wxService.switchoverTo(appid);
        if (!wxService.checkSignature(timestamp, nonce, signature)) {
            throw new IllegalArgumentException("[MP]非法请求，可能属于伪造的请求！");
        }
