...
import com.github.zj.dreamly.modules.wx.dao.MsgReplyRuleMapper;
...
public class MsgReplyRuleServiceImpl extends ServiceImpl<MsgReplyRuleMapper, MsgReplyRule>
    ...
    private final MsgReplyRuleMapper msgReplyRuleMapper;
    ...
    public List<MsgReplyRule> getMatchedRules(String appid, boolean exactMatch, String keywords) {
        ...
        return this.getValidRules().stream()
                ...
                .filter(rule -> StringUtils.isEmpty(rule.getAppid()) || appid.equals(rule.getAppid()))
                ...
                //检测是否符合匹配规则
                .filter(rule -> isMatch(exactMatch || rule.isExactMatch(),
                        rule.getMatchValue().split(","), keywords))
